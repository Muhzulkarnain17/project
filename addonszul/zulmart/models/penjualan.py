...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='zulmart.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for line in self:
            result = sum(self.env['zulmart.detailpenjualan'].search(
                [('penjualan_id', '=', line.id)]).mapped('subtotal'))
            line.total_bayar = result


class DetailPenjualan(models.Model):
    _name = 'zulmart.detailpenjualan'
    _description = 'Detail'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(
        comodel_name='zulmart.penjualan',
        string='Detail Penjualan')
    barang_id = fields.Many2one(
        comodel_name='zulmart.barang',
        string='List Barang')
    ...
    def create(self, vals):
       ...
       if line.qty:
        ...
        self.env['zulmart.barang'].search(
...
def __ondelete_penjualan(self):
    if self.detailpenjualan_ids:
        penjualan = []
        for line in self:
            penjualan = self.env['zulkarnain.detailpenjualan'].search(
...
def unlink(self):
    if self.detailpenjualan_ids:
        penjualan = []
        for line in self:
            penjualan = self.env['zulmart.detailpenjualan'].search(
                [('penjualan_id', '=', line.id)])
            print(penjualan)
